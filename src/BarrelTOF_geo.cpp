 ...
 * @author Zhenyu Ye
 */
static Ref_t create_BarrelTOF(Detector& description, xml_h e, SensitiveDetector sens) {
  ...
  for (xml_coll_t mi(x_det, _U(module)); mi; ++mi) {
    ...
    if (volumes.find(m_nam) != volumes.end()) {
      printout(ERROR, "BarrelTOF", string((string("Module with named ") + m_nam + string(" already exists."))).c_str() );
    ...
    for (xml_coll_t ci(x_mod, _U(module_component)); ci; ++ci, ++ncomponents) {
      ...
      double     dz     = getAttrOrDefault(x_comp, _U(dz), 0.0 * mm);
      ...
      thickness_sum += x_comp.thickness()+dz;
      thickness_so_far += x_comp.thickness()+dz;
    }
  }
...
//@}
// clang-format off
DECLARE_DETELEMENT(BarrelTOF, create_BarrelTOF)
DECLARE_DETELEMENT(athena_TOFBarrel,   create_BarrelTOF)
